...
import { readJson } from '$lib/server/scan/http';
...
export const POST: RequestHandler = async ({ request }) => {
	...
	const pages: PageProvider[] = body.pageIds.map((pageId) => async () => {
		const { artefactPath } = scanPagePaths(body.sessionId, pageId);
	...
	await dropScanSession(body.sessionId);
