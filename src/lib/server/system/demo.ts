...
import { seedDemoLife } from './demo-life';
...
export async function seedDemo(): Promise<void> {
	...
	await fileDemoPdf({
		...
		lines: [
			`Employer: ${DEMO_EMPLOYER}`,
			`Employee: ${JANA}`,
			`Tax year: ${declaredYear}`,
			'',
			'Taxpayer declaration — signed, on file.',
			'',
			'Fictional document, generated by the Continuum demo seed.'
		]
	});
	...
	// And one that runs out too soon to travel on.
	//
	// Deliberately dated five months from today, which is inside the six months
	// most borders ask for beyond the day somebody comes home. It is the case
	// the Trips readiness block exists to catch: valid on the day, and refused
	// at the desk anyway. Without it the demo shows only green pills and the
	// block reads as decoration.
	const expiringPassport = new Date();
	expiringPassport.setUTCMonth(expiringPassport.getUTCMonth() + 5);
	await fileDemoPdf({
		name: `Passport · ${JANA}`,
		shelfKey: 'identity',
		type: 'id_document',
		targetIds: [jana],
		expiresOn: expiringPassport.toISOString().slice(0, 10),
		identity: {
			kind: 'passport',
			country: 'CZ',
			number: 'CZ 38 114 620',
			issuedOn: `${thisYear - 10}-04-19`,
			issuer: 'Ministerstvo vnitra ČR'
		},
		lines: [
			`Holder: ${JANA}`,
			'Number: CZ 38 114 620',
			`Issued: ${thisYear - 10}-04-19`,
			`Expires: ${expiringPassport.toISOString().slice(0, 10)}`,
			'',
			'Fictional travel document, generated by the Continuum demo seed.'
		]
	});
	...
	// The half of this household that is not its money: trips, the map they
	// fill in, the cookbook and the cellar. In its own module because this file
	// is already the longest in the project, and last because every row in it
	// hangs off the two people above.
	await seedDemoLife({ jana, petr });
}
