<script lang="ts">
	...
	/** A new shelf's template, which decides what units it may be organised by. */
	let newTemplate = $state<ShelfTemplate>('obligations');

	/** What each unit is called where a person picks it. */
		...
		<form class="shelf-dialog" method="POST" action="?/addShelf" use:enhance>
			...
			<label class="shelf-field">
				...
				<select name="template" bind:value={newTemplate}>
					{#each ENUMS['shelf.template'] as t (t)}
						<option value={t}>{TEMPLATE_LABELS[t]}</option>
					{/each}
				</select>
				<span class="quiet">{TEMPLATE_BLURBS[newTemplate]}</span>
			</label>

			<label class="shelf-field">
				<span class="eyebrow">Organised by</span>
				<select name="unit">
					{#each unitsForTemplate(newTemplate) as u (u)}
			...
			<label class="shelf-field">
				<span class="eyebrow">The question it answers</span>
				<input name="question" placeholder="Is she seaworthy?" />
			</label>
