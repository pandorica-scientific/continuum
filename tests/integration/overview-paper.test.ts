...
/**
 * The Paper panel's counts.
 *
 * The builder reads the module-level `db` singleton rather than a handle, so it
 * has to be pointed at this harness the way the other document suites do it.
 */
...
/** The panel, through the loader the screen itself uses. */
async function paperFor(): Promise<PaperPanel> {
...
describe('the Paper panel', () => {
	it('counts every document on a shelf', async () => {
		await seed({ name: 'Passport', shelf: 'identity', expiresOn: soon });
		await seed({ name: 'Divorce papers', shelf: 'identity', expiresOn: soon });

		const panel = await paperFor();

		expect(panel.expiring.soon).toBe(2);
