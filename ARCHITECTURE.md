...
multi-row changes to modules under `src/lib/server/<domain>/` (every domain is a
directory with an `index.ts`; there are no loose files at that level, and a test
says so). The domain function owns the
...
- `src/lib/server/` — everything with database or network access. Directories
  only: a domain per directory, and `system/` for the plumbing none of them owns
