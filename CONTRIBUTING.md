...
`linux/arm64` and pushes `0.8.3` alongside `latest` to both registries.

Two things fail the build rather than publishing something wrong:

- **The tag must match `package.json`.** `v0.8.3` against a `0.8.2` package is
...
`Publishing:` line reads `***/continuum:0.8.3` rather than naming the account.
