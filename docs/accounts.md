...
The setup wizard makes its first person an **administrator**; anyone added later is
...
password hints in the interface are fed by the same value the server enforces, so
...
On the plain-HTTP LAN address that is everyone on the network — see
...
database — names are not unique, so check the name matches only the person you
mean:
